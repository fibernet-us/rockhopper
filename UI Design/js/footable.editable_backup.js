	...
	function addButtonsToRow(row, buttons) {
        $(buttons.buttonCols).each(function (i) {
            if ($(row).find('td').eq(buttons.buttonCols[i]).length > 0) {
                $(row).find('td').eq(buttons.buttonCols[i])(function () {
                    if ($(this).find('"i" .icon-trash').length <= 0) {
                        $(this).append('<a class="btn btn-mini" type="button" value="Delete"><i class="icon-trash"></i></a>');
                    }
                    if ($(this).find('"i" .icon-pencil').length <= 0) {
                        $(this).append('<a class="btn btn-mini" type="button" value="Edit"><i class="icon-pencil"></i></a>');
                    }
                });
            } else {
                $(row).append('<td><a class="btn btn-mini" type="button" value="Delete"><i class="icon-trash"></i></a><a class="btn btn-mini" type="button" value="Edit"><i class="icon-pencil"></i></a></td>');
            }
        });
...
/*
        ...
        if (row.hasClass('fooNewRecord')) {
            if (!addButtonColVisible && buttons.addCols.length >= 0) {
                if ($(detailDiv).find('input[type="button"][value="Add"]').length <= 0) {
                    ...
                    $(detailDiv).find('"a"[type="button"][value="Delete"]').remove();
                }
            }
        }
        else {
            if (!newRecordRow && !deleteButtonColVisible && buttons.deleteCols.length >= 0) {
                if ($(detailDiv).find('input[type="button"][value="Delete"]').length <= 0) {
                    $(detailDiv).append('<"a" class="btn btn-mini" type="button" value="Delete"><i class="icon-trash"></i></a>');
                    $(detailDiv).find('input[type="button"][value="Add"]').remove();
                }
            }
        }
    }
	*/
    ...
    function processServerResponse(target, data, updateRecord) {
		...
		/*---------- Deleted hasclass('btn-mini'), may be wrong! ---------------*/ 
            ...
            $(ft.table).bind({
                'footable_initialized': function (e) {
                    ...
                    $(ft.table).on('click', '.icon-pencil', function (e) {
						...
						$(curRow).find('td:last').attr('contentEditable', false);
                    });
